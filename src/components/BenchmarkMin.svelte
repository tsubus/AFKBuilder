<script>
  ...
  onMount(async () => {
    $AppData.activeView = "benchmarkmin";
  ...
  function buildRecs() {
    ...
    for (let i = 0; i < $HeroData.length; i++) {
      ...
      if ($AppData.MH.List[hero.id].claimed) {
        if (
          $AppData.MH.List[hero.id].si < hero.si_benchmark_min &&
          hero.si_benchmark_min > 0
        ) {
          buffer.push({
            id: hero.id,
            type: "si",
            value: { si: hero.si_benchmark_min },
          });
        }
        if ($AppData.MH.List[hero.id].furn < hero.furn_benchmark_min) {
          buffer.push({
            id: hero.id,
            type: "furn",
            value: { furn: hero.furn_benchmark_min },
          });
        }
        if (
          $AppData.MH.List[hero.id].engraving < hero.engraving_benchmark_min
        ) {
          buffer.push({
            id: hero.id,
            type: "engraving",
            value: {
              engraving: hero.engraving_benchmark_min,
