<script>
  ...
  import {
    gql_UPDATE_MY_HEROES,
    gql_UPDATE_LOCAL_COMPS,
  } from "./gql/queries.svelte";
  ...
  async function populateUserData() {
    ...
    if (user.my_heroes) {
      if ("lastUpdate" in user.my_heroes)
        user.my_heroes.lastUpdate = new Date(user.my_heroes.lastUpdate);
    }
    if (user.local_comps) {
      if ("lastUpdate" in user.local_comps)
        user.local_comps.lastUpdate = new Date(user.local_comps.lastUpdate);
    }
  ...
  async function syncFavoriteComps() {
    ...
    if ($AppData.user.saved_comps.length > 0) {
      for (const saveComp of $AppData.user.saved_comps) {
        if (
          !$AppData.user.published_comps.some((e) => e.uuid === saveComp.uuid)
        ) {
          // favorited comp is not one of the user's own published comps
          let needsUpdate = true;
          let curCompExists = $AppData.Comps.some(
            (e) => e.uuid === saveComp.uuid
          );
          let curIdx;
          if (curCompExists) {
            ...
            needsUpdate =
              $AppData.Comps[curIdx].lastUpdate < saveComp.comp_update;
          }
          if (needsUpdate) {
            ...
            if (svrComp.status !== 200) {
              console.log(
                `Unable to sync favorited comp with uuid: ${saveComp.uuid}`
              );
            ...
            if (svrComp.data.length === 0) {
              ...
              $AppData.user.saved_comps = $AppData.user.saved_comps.filter(
                (e) => e.uuid !== saveComp.uuid
              );
            ...
            try {
              const json = await jsurl.decompress(
                svrComp.data[0].attributes.comp_string
              );
              compData = JSON.parse(json);
            } catch (err) {
              console.log(
                `Unable to parse favorited comp with uuid: ${saveComp.uuid}`
              );
              console.log(err);
              displayNotice({ type: "error", message: "Favorite sync failed" });
              continue;
            }
            if ("lastUpdate" in compData)
              compData.lastUpdate = new Date(compData.lastUpdate);

            // validate resulting data is good
            const returnObj = await validateComp(compData);
            if (returnObj.retCode !== 0) {
              console.log(
                `Unable to validate favorited comp with uuid: ${saveComp.uuid}`
              );
              displayNotice({ type: "error", message: "Favorite sync failed" });
              continue;
            }
  ...
  async function syncMyHeroes() {
    ...
    if ($AppData.user.my_heroes && "lastUpdate" in $AppData.user.my_heroes) {
      ...
      if ($AppData.user.my_heroes.lastUpdate > $AppData.MH.lastUpdate) {
        ...
        $AppData.user.my_heroes.lastUpdate.getTime() ===
        $AppData.MH.lastUpdate.getTime()
      ) {
    ...
    switch (updateType) {
      case "push":
        ...
        try {
          ...
          const response = await gqlUpdateMyHeroes({
            variables: {
              id: $AppData.user.id,
              mh: { lastUpdate: $AppData.MH.lastUpdate, data: mhData },
            },
          });
          const newMyHeroes =
            response.data.updateUsersPermissionsUser.data.attributes.my_heroes;
      ...
      default:
        throw new Error(
          `Error, invalid updateType computed in syncMyHeroes something really went wrong. ${updateType}`
        );
    }
  }
  ...
  async function syncLocalComps() {
    ...
    if (
      $AppData.user.local_comps &&
      "lastUpdate" in $AppData.user.local_comps
    ) {
      ...
      if ($AppData.user.local_comps.lastUpdate > $AppData.compLastUpdate) {
        ...
        $AppData.user.local_comps.lastUpdate.getTime() ===
        $AppData.compLastUpdate.getTime()
      ) {
        // server and local are sync'd no need to update
        updateType = "none";
      } // else local is ahead of server, so do a push that's already set
    ...
    switch (updateType) {
      case "push":
        try {
          await displayNotice({ type: "loading" });
          const localComps = $AppData.Comps.filter(
            (e) => e.source === "local" && !e.hidden
          );
          const compData = await jsurl.compress(JSON.stringify(localComps));
          const groupData = await jsurl.compress(
            JSON.stringify($AppData.compGroups)
          );
          const response = await gqlUpdateLocalComps({
            variables: {
              id: $AppData.user.id,
              comps: {
                lastUpdate: $AppData.compLastUpdate,
                data: compData,
                groups: groupData,
              },
            },
          ...
          const newLocalComps =
            response.data.updateUsersPermissionsUser.data.attributes
              .local_comps;
      ...
      case "pull":
        ...
        try {
          ...
          const groupJSON = await jsurl.decompress(
            $AppData.user.local_comps.groups
          );
        ...
        for (const comp of compsData) {
          ...
          if ($AppData.Comps.some((e) => e.uuid === returnObj.message.uuid)) {
            ...
            const idx = $AppData.Comps.findIndex(
              (e) => e.uuid === returnObj.message.uuid
            );
            if (!$AppData.Comps[idx].hidden)
    ...
    if (urlParams.has("modal")) {
      ...
      window.addEventListener("popstate", function updateURL(event) {
        const uri = $querystring
          ? `${window.location.origin}/#${$location}?${$querystring}`
          : `${window.location.origin}/#${$location}`;
        history.replaceState(
          { view: $AppData.activeView, modal: false },
          $AppData.activeView,
          uri
        );
  ...
  async function changeColorProfile(profile) {
    ...
    switch (profile) {
      case "light":
        ...
        root.style.setProperty(
          "--neu-convex-BGLight-bg",
          "linear-gradient(145deg, #ffffff, #ebebeb)"
        );
        root.style.setProperty(
          "--neu-convex-BGColor-bg",
          "linear-gradient(145deg, #ffffff, #d8d8da)"
        );
        root.style.setProperty(
          "--neu-convex-BGColor-wide-bg",
          "linear-gradient(160deg, #ffffff, #d8d8da)"
        );
        root.style.setProperty(
          "--neu-concave-BGColor-bg",
          "linear-gradient(145deg, #d8d8da, #ffffff)"
        );
        root.style.setProperty(
          "--neu-large-ni-BGColor-shadow",
          "8px 8px 20px #ccccce, 8px -8px 20px #ffffff"
        );
        root.style.setProperty(
          "--neu-med-i-BGLight-shadow",
          "5px 5px 10px #d1d1d1, -5px -5px 10px #ffffff"
        );
        root.style.setProperty(
          "--neu-med-i-BGLight-hover-shadow",
          "8px 8px 16px #d1d1d1, -8px -8px 16px #ffffff"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-shadow",
          "5px 5px 8px #ccccce, -5px -5px 8px #ffffff"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-hover-shadow",
          "8px 8px 16px #ccccce, -8px -8px 16px #ffffff"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-pressed-shadow",
          "3px 3px 5px #ccccce, -3px -3px 5px #ffffff"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-inset-shadow",
          "inset 5px 5px 8px #ccccce, inset -5px -5px 8px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-i-BGColor-shadow",
          "5px 5px 5px #ccccce, -5px -5px 5px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-i-BGLight-shadow",
          "5px 5px 5px #d1d1d1, -5px -5px 5px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-i-BGColor-hover-shadow",
          "8px 8px 10px #ccccce, -8px -8px 10px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-i-BGColor-pressed-shadow",
          "3px 3px 3px #ccccce, -3px -3px 3px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-i-BGLight-pressed-shadow",
          "3px 3px 3px #d1d1d1, -3px -3px 3px #ffffff"
        );
        root.style.setProperty(
          "--neu-med-ni-BGColor-shadow",
          "5px 5px 8px #ccccce, -5px -5px 8px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-ni-BGColor-shadow",
          "5px 5px 5px #ccccce, -5px -5px 5px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-ni-BGColor-pressed-shadow",
          "3px 3px 3px #ccccce, -3px -3px 3px #ffffff"
        );
        root.style.setProperty(
          "--neu-sm-ni-BGColor-inset-shadow",
          "inset 3px 3px 3px #ccccce, inset -3px -3px 3px #ffffff"
        );
      ...
      case "dark":
        ...
        root.style.setProperty(
          "--neu-convex-BGLight-bg",
          "linear-gradient(145deg, #485165, #3c4455)"
        );
        root.style.setProperty(
          "--neu-convex-BGColor-bg",
          "linear-gradient(145deg, #313844, #292f3a)"
        );
        root.style.setProperty(
          "--neu-convex-BGColor-wide-bg",
          "linear-gradient(160deg, #313844, #292f3a)"
        );
        root.style.setProperty(
          "--neu-concave-BGColor-bg",
          "linear-gradient(145deg, #292f3a, #313844)"
        );
        root.style.setProperty(
          "--neu-large-ni-BGColor-shadow",
          "8px 8px 20px #272c36, 8px -8px 20px #353c4a"
        );
        root.style.setProperty(
          "--neu-med-i-BGLight-shadow",
          "5px 5px 10px #394150, -5px -5px 10px #4d576c"
        );
        root.style.setProperty(
          "--neu-med-i-BGLight-hover-shadow",
          "8px 8px 16px #394150, -8px -8px 16px #4d576c"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-shadow",
          "5px 5px 8px #272c36, -5px -5px 8px #353c4a"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-hover-shadow",
          "8px 8px 16px #272c36, -8px -8px 16px #353c4a"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-pressed-shadow",
          "3px 3px 5px #272c36, -3px -3px 5px #353c4a"
        );
        root.style.setProperty(
          "--neu-med-i-BGColor-inset-shadow",
          "inset 5px 5px 8px #272c36, inset -5px -5px 8px #353c4a"
        );
        root.style.setProperty(
          "--neu-sm-i-BGColor-shadow",
          "5px 5px 5px #272c36, -5px -5px 5px #353c4a"
        );
        root.style.setProperty(
          "--neu-sm-i-BGLight-shadow",
          "5px 5px 5px #394150, -5px -5px 5px #4d576c"
        );
        root.style.setProperty(
          "--neu-sm-i-BGColor-hover-shadow",
          "8px 8px 10px #272c36, -8px -8px 10px #353c4a"
        );
        root.style.setProperty(
          "--neu-sm-i-BGColor-pressed-shadow",
          "3px 3px 3px #272c36, -3px -3px 3px #353c4a"
        );
        root.style.setProperty(
          "--neu-sm-i-BGLight-pressed-shadow",
          "3px 3px 3px #394150, -3px -3px 3px #4d576c"
        );
        root.style.setProperty(
          "--neu-med-ni-BGColor-shadow",
          "5px 5px 8px #272c36, -5px -5px 8px #353c4a"
        ...
        root.style.setProperty(
          "--neu-sm-ni-BGColor-inset-shadow",
          "inset 3px 3px 3px #272c36, inset -3px -3px 3px #353c4a"
        );
        $AppData.colorProfile = profile;
        break;
      default:
  ...
  async function handleRouteEvent(event) {
    switch (event.detail.action) {
      ...
      default:
        throw new Error(
          `Invalid action specified for route event: ${event.detail.action}`
        );
    }
  }
  ...
  async function displayNotice(notice_config) {
    ...
    if (noticeConf.type !== "loading")
      noticeTimeout = setTimeout(() => clearNotice(), 2000);
  }
</script>

<svelte:head>
  <title>AFKBenchmarks</title>
