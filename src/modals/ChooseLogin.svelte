<script>
  ...
  onMount(async () => {
    history.pushState(
      { view: $AppData.activeView, modal: true },
      "Choose Login",
      `?modal=true${window.location.hash}`
    );
  });
    ...
    <div class="loadingSpinnerContainer">
      <LoadingSpinner
        ...
        color={window
          ...
          .getPropertyValue("--appColorPrimary")}
      />
    </div>
  {:else}
    <h4>Login with:</h4>
    <ul>
      {#each providers as provider}
        <li>
          <button
            type="button"
            class="{provider.name.toLowerCase()}Button providerButton"
            on:click={() => handleProviderClick(provider.uri)}
          >
            <img class="providerLogo" src={provider.logo} alt={provider.name} />
            <img
              class="providerLogoAlt"
              src={provider.logoAlt}
              alt={provider.name}
            />
            <span class="{provider.name.toLowerCase()}Text providerText"
              >{provider.name}</span
            >
          </button>
        </li>
